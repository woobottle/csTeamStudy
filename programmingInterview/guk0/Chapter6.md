...
- 이 경우, 너비 우선 검색(BFS), 깊이 우선 검색(DFS)을 사용
  - 너비 우선 검색
    ...
    - O(n)이고 어떤 층을 검색할 때 그 층에 있는 모든 노드의 자식 노드를 저장해둬야 하기 때문에 메모리 사용량이 높음. ***깊이 우선이랑 메모리차이 왜? -> dfs는 스택을 안쓰고 재귀적으로 풀 수 있는데 bfs는 큐를 써야해서? 둘다 노드를 방문했는지 안했는지 여부를 저장하는건 동일함.***
...
- bfs와 dfs가 메모리 차이나는 이유. 
  - bfs는 큐를 사용, dfs는 스택을 사용.
  - DFS는 지금 현재 탐색하는 경로만 기억하면 되므로 선형 공간을 차지하므로 비교적 공간 사용이 작다
    - 스택을 사용하냐 큐를 사용하냐의 차이인듯?
...
<img src="https://user-images.githubusercontent.com/72545106/139589131-f6f22876-3775-40d0-a480-8e1256c3a3e5.png"  alt="graph" width="300" />
